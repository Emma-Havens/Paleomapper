...
class Figure:
    ...
    def set_color_list(self):
        ...
        self.plateid_color_list = ["white", "red", "mediumpurple", "limegreen", "orange", 
                                    "orchid", "royalblue", "yellow", "cyan", "gray"]
        # self.plateid_color_lightness = [.35, .40, .45, .50, .55, .60, .65, .70, .75, .80]
        self.plateid_color_lightness = [.50, .55, .65, .70, .72, .74, .78, .80, .75,.82, .84]
    ...
    def plot_rasters(self, raster_list):
        for raster in raster_list:
            _, _, file_path, raster_extent, raster_alpha = raster
    ...
    def plot_to_screen(self, chunk_generator):
        ...
        self.plot_rasters(self.rasters)
